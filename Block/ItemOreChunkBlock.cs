...
namespace Mekiasm
{
    public class ItemOreChunkBlock : Block
    {
        public const int Index = 1005;
        public ItemOreChunkBlock()
        {
        }
        ...
        public virtual Item GetItem(ref int value)
        {
            ...
            if (Terrain.ExtractContents(value) != 1005) return null;
            return MekiasmInit.items_orechunk.Where(p => p.itemid == id).FirstOrDefault();
        }
        ...
        {
            int[] arr = new int[MekiasmInit.items_orechunk.Count];
            for (int i = 0; i < MekiasmInit.items_orechunk.Count; i++)
            {
                arr[i] = Terrain.MakeBlockValue(1005, 0, MekiasmInit.items_orechunk.Array[i].itemid);
            }
            return arr;
        }
